    ...
    return `
      <div class="bg-neutral-950 border border-neutral-800 p-3 mb-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          ${omField('É sobrado?', `<select id="om-prop-sis-sobrado" class="${OM_INPUT_CLS}">
            <option value="">— não informado</option>
            <option value="sim"${p.sobrado === true ? ' selected' : ''}>Sim</option>
            <option value="nao"${p.sobrado === false ? ' selected' : ''}>Não</option>
          </select>`)}
        </div>
...
async function omPropSisResolve(clienteId) {
  ...
  const sobradoRaw = val('om-prop-sis-sobrado');
  const ehSobrado = sobradoRaw === 'sim' ? true : sobradoRaw === 'nao' ? false : null;
  const algumPreenchido = [apelido, pot, qtdmod, marcamod, modelomod, marcainv, modeloinv, telhado, sobradoRaw].some(Boolean) || omPropSis.origemId;
...
function omPropSistemaCardHTML(s) {
  ...
  const cells = [
    ...
    ['building-2', 'Sobrado', s.eh_sobrado === true ? 'Sim' : s.eh_sobrado === false ? 'Não' : null],
  ].filter(c => c[2]);
...
function renderOMRoute(container, tabId) {
  ...
  switch (tabId) {
    ...
    case 'agenda':     return (typeof omRenderAgendaTab === 'function') ? omRenderAgendaTab(container) : renderOMCentral(container);
