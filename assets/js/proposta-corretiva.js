// ==========================================
// PROPOSTA MANUTENÇÃO CORRETIVA — PÁGINA PÚBLICA
// Clone da proposta O&M, adaptado para a narrativa de reparo:
//   Problema identificado → Diagnóstico/causa → Solução (reparo)
...
// ----------------------------------------------------------------------
// Escopo padrão do reparo (fallback quando a proposta não traz `incluso`/`nao_incluso`).
// Focado em corretiva — o serviço "Manutenção corretiva" já traz esses itens por padrão.
// ----------------------------------------------------------------------
...
// Preenche um card de texto livre (problema/diagnóstico) e esconde o bloco quando vazio.
function setCardText(blocoId, textId, value) {
  const bloco = $(blocoId);
  const el = $(textId);
  const v = (value == null ? '' : String(value)).trim();
  if (!bloco || !el) return;
  if (v) { el.textContent = v; bloco.style.display = ''; }
  else   { bloco.style.display = 'none'; }
}

// ----------------------------------------------------------------------
// Parcelamento no cartão (corretiva: até 12x)
// ----------------------------------------------------------------------
const TAXAS_CARTAO = {
...
// ----------------------------------------------------------------------
function renderListaInclusos(incluso) {
...
function render(p) {
  const tipo   = p.tipo || 'Manutenção Corretiva';
  ...
  if (subEl) subEl.textContent = 'Preparamos esta proposta de manutenção sob medida para o seu sistema fotovoltaico.';
  ...
  // Problema identificado (descricao) + Diagnóstico / causa provável (corretiva_diagnostico)
  setCardText('bloco-problema', 'corretiva-problema', p.descricao);
  setCardText('bloco-diagnostico', 'corretiva-diagnostico', p.diagnostico || p.corretiva_diagnostico);

  // O que está incluído
  renderListaInclusos(p.incluso);
  ...
  $('cover-sub').textContent = `Reparo do sistema fotovoltaico, preparado para o ${p.cliente || 'cliente'}.`;
    ...
    if (finalBox) {
      if (p.status === 'Aprovada') {
        ...
        finalBox.innerHTML = '<div class="ok"><i data-lucide="check"></i></div>'
          ...
          + '<p>Recebemos seu aceite. A equipe da Ágil Solar entra em contato em breve para agendar o reparo.</p>';
      } else {
...
function revealDocument() {
  ...
  if (typeof captureEvent === 'function') {
    try { captureEvent('public_om_proposal_opened', { mode: mockId ? 'mock' : 'live', kind: 'corretiva' }); } catch (_) {}
  }
}
...
// ----------------------------------------------------------------------
// Persistência real — Supabase RPC (mesma tabela/RPC do O&M)
// ----------------------------------------------------------------------
...
function boot() {
  ...
  const tryFinish = () => {
    ...
    render(data);
    if (typeof captureEvent === 'function') {
      try { captureEvent('public_om_proposal_viewed', { mode: mockId ? 'mock' : 'live', kind: 'corretiva' }); } catch (_) {}
    }
    revealDocument();
  };
